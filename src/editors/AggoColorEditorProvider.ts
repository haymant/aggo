...
export class AggoColorEditorProvider implements vscode.CustomTextEditorProvider {
  ...
  public async resolveCustomTextEditor(document: vscode.TextDocument, webviewPanel: vscode.WebviewPanel, _token: vscode.CancellationToken): Promise<void> {
    ...
    const bridge = attachFileBridgeHandler({ webviewPanel, document, openWithEditor: 'aggo.colorEditor', preferDocumentDir: true });
    ...
    webviewPanel.webview.onDidReceiveMessage(async (msg: any) => {
      if (msg.type === 'ready') {
        ...
        } catch (e) { console.warn('[aggo] failed applying update for color editor', e); }
      } else if (msg.type === 'requestSave') {
        try { await document.save(); } catch (e) { console.warn('[aggo] failed saving color editor doc', e); }
      }
    });
...
export default AggoColorEditorProvider;
