...
export class AggoDataSourceEditorProvider implements vscode.CustomTextEditorProvider {
  ...
  public async resolveCustomTextEditor(document: vscode.TextDocument, webviewPanel: vscode.WebviewPanel, _token: vscode.CancellationToken): Promise<void> {
    ...
    const bridge = attachFileBridgeHandler({ webviewPanel, document, openWithEditor: 'aggo.dataSourceEditor', preferDocumentDir: true });
    ...
    webviewPanel.webview.onDidReceiveMessage(async (msg: any) => {
      if (msg.type === 'ready') {
        ...
        } catch (e) { console.warn('[aggo] failed applying update for datasource editor', e); }
      } else if (msg.type === 'requestSave') {
        try { await document.save(); } catch (e) { console.warn('[aggo] failed saving datasource editor doc', e); }
      }
    });
...
export default AggoDataSourceEditorProvider;
